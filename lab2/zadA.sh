...
declare -A map_of_words

files=$(cat $( ls ddd/nazwy-? ))

for file in ${files}; do

    if [ "${map_of_words[${f}]}" ]; then
        map_of_words[${file}]=$((${map_of_words[${file}]} + 1))
    else
        map_of_words[${file}]=0
    fi
...
for file in ${!map_of_words[@]}; do
 
    echo ${map_of_words[${file}]} > "ddd/zasoby/${file}"
